...
namespace KCore
{
    ...
    public abstract class BaseForm
    {
        ...
        public BaseForm Reference { get; set; } = null;
        ...
        public void Start(BaseForm form, bool redirected = false, bool other_form_will_be_start = false)
        {
            form.Reference = this;
        ...
        #region Form Finding
        private static Type FormType { get; } = typeof(BaseForm);

        private BaseForm InternalFindFirst(Type type)
        {
            if (Reference != null)
            {
        ...
        private BaseForm InternalFindFirstOrDefault(Type type)
        {
            if (Reference != null)
            {
                if (Reference.GetType() == type) return Reference;
        ...
        protected BaseForm FindFirst(Type type)
        {
        ...
        protected BaseForm FindFirstOrDefault(Type type)
        {
            if (!IsFormType(type)) throw new Exception("BaseType is not Form");
        ...
        protected static BaseForm DashboardForm = new Dashboard();
        protected static BaseForm ExceptionForm;
